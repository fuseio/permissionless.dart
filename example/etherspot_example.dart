// Example: Creating and using an Etherspot smart account with permissionless.dart
//
// This example demonstrates:
// 1. Creating an Etherspot ModularEtherspotWallet (EntryPoint v0.7)
// 2. Sending a real UserOperation on Sepolia testnet
// 3. Both sponsored (Pimlico paymaster) and self-funded modes
//
// USAGE:
//   dart run example/etherspot_example.dart              # Sponsored (free gas)
//   dart run example/etherspot_example.dart --self-fund  # Self-funded (requires ETH)
//
// REQUIREMENTS:
...
void main(List<String> args) async {
  ...
  print('='.padRight(60, '='));
  print('Etherspot ModularEtherspotWallet Example');
  ...
  // ================================================================
  // 1. Create an Etherspot Smart Account
  // ================================================================
  //
  // ModularEtherspotWallet features:
  // - ERC-7579 compliant modular architecture
  // - EntryPoint v0.7 support
  // - Pluggable validators, executors, and fallback handlers
  // - Bootstrap initialization for module configuration

  const rpcUrl = 'https://eth-sepolia.g.alchemy.com/v2/YOUR_KEY';
  final publicClient = createPublicClient(url: rpcUrl);

  // Pass publicClient so the account can compute its address via RPC
  // (same pattern as permissionless.js)
  final account = createEtherspotSmartAccount(
    owner: owner,
    chainId: BigInt.from(11155111), // Sepolia
    index: BigInt.zero, // Salt for address derivation
  ...
  // Address is computed automatically via RPC on first access
  final accountAddress = await account.getAddress();
  final isDeployed = await publicClient.isDeployed(accountAddress);
  print(
    'Etherspot Account: ${accountAddress.checksummed} ${isDeployed ? "(already deployed)" : "(will be deployed)"}',
  );
  ...
  print('Account type: Etherspot ModularEtherspotWallet');
